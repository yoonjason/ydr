...
후보 D(미등록 보스 자동 경보)는 독립적인 인프라와 오탐 리스크 관리가 필요하므로 별도 Phase 로 분리해 충분한 실기 데이터가 쌓인 이후 착수하는 것이 안전하다. 후보 E(파티 힐러 분담)는 **Phase 5 시퀀스에서 제외** — 제품 주 사용 시나리오인 5인 M+ 는 힐러 1명 고정이므로 기능의 적용 대상이 구조적으로 존재하지 않는다. 레이드(10/20인) 지원이 제품 범위에 편입되는 Phase 6+ 에 재평가한다.
...
| ~~E. 파티 힐러 분담~~ | ~~3~~ | ~~3~~ | ~~3~~ | ~~9~~ | **Phase 6+ 레이드 확장으로 격하** |

*주의*: 합산이 낮아도 A > B 를 권장하는 이유는 A 가 B 의 품질(알림 relevance)을 높이는 기반 데이터를 제공하기 때문.

*후보 E 격하*: 제품 주 사용 시나리오(5인 M+)는 힐러 1명 구조. 레이드 지원이 제품 범위에 편입되기 전까지는 Phase 5 시퀀스에서 제외.
...
**~~Phase 5d~~ → Phase 6+ — 파티 힐러 분담 (1~2일)**
**Phase 5 시퀀스에서 제외.** 제품 주 대상인 5인 M+ 는 힐러 1명 고정 구조이므로 이 기능의 적용 대상이 구조적으로 존재하지 않는다. 향후 레이드(10/20인) 지원이 제품 범위에 편입되는 시점(Phase 6+)에 재평가.
